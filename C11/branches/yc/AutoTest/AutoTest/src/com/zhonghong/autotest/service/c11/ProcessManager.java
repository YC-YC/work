...
import com.zhonghong.autotest.client.BTClient;
import com.zhonghong.autotest.client.GPSManager;
import com.zhonghong.autotest.client.LogicClient;
import com.zhonghong.autotest.client.RadioClient;
 ...
 * TODO:数据处理
 */
public class ProcessManager implements ILifeCycle{
	...
	private Set<IATClient> mClients = new HashSet<IATClient>();
	private LogicClient mLogicClient;
	private RadioClient mRadioClient;
	private BTClient mBTClient;
	private GPSManager mGPSGpsManager;
	
	public ProcessManager(ATService service) {
		super();
		this.mService = service;
		mLogicClient = new LogicClient(this);
		mRadioClient = new RadioClient(this);
		mBTClient = new BTClient(this);
		mGPSGpsManager = new GPSManager(this);
	}
	...
	public void onObjectCreate() {
		mLogicClient.onObjectCreate();
		mRadioClient.onObjectCreate();
		mBTClient.onObjectCreate();
		mGPSGpsManager.onObjectCreate();
	}

	@Override
	public void onObjectDestory() {
		mLogicClient.onObjectDestory();
		mRadioClient.onObjectDestory();
		mBTClient.onObjectDestory();
		mGPSGpsManager.onObjectDestory();
	}
	
	public boolean registerClient(IATClient client){
		synchronized (mLock) {
			if (!mClients.contains(client)){
				mClients.add(client);
				Log.i(TAG, "registerClient had clients size = " + mClients.size());
			}
			else{
				Log.i(TAG, "had exit client");
				return false;
			}
			return true;
		}
	...
	public boolean unregisterClient(IATClient client){
		synchronized (mLock) {
			if (mClients.contains(client)){
				mClients.remove(client);
				Log.i(TAG, "unregisterClient had clients size = " + mClients.size());
			}
			else{
				Log.i(TAG, "client not exit");
				return false;
			}
			return true;
		}
	...
	public byte[] genCANData(byte[] data, int len/*, int type*/){
		...
		canData[len+3] = getCheckSun(canData, 1, len+2);
		return canData;
	}
	 ...
	 * @param caninfo
	 */
	public void onReceiveCanData(byte[] caninfo){
		ATEvent atEvent = onProcessConvert(caninfo);
		if (atEvent != null){
			onDispatch(atEvent);
		}
	}
	 ...
	 * @param info
	 */
	public void onResponseFromClient(ATEvent atEvent){
		byte[] data = onProcessRevert(atEvent.type, atEvent.cmd, atEvent.info);
		if (data != null){
			mService.sendData(data);
		}
	}

	private void onDispatch(ATEvent atEvent){
		synchronized (mLock) {
			Iterator<IATClient> iterator = mClients.iterator();
			while(iterator.hasNext()){
				iterator.next().onDispatch(atEvent);
			}
		}
	}
	 ...
	 * @return
	 */
	private ATEvent onProcessConvert(final byte[] caninfo){
		//TODO 协议处理
		int id = (caninfo[3]&0xFF);
		Log.i(TAG, "onProcessConvert id = " + id);
		byte len = caninfo[4];
		switch (id) {
		case AT_ID.ID_TRUN_OFF: {
			mService.sendATBroadcast(ATConst.Broadcast.KEY_END_AT, "true");
			byte[] data = { 0x02, 0x00 };
			mService.sendData(genCANData(data, data.length));
			mService.stopSelf();
		}
			break;
...
//			mService.sendData(genCANData(data, data.length, TYPE_POSITIVE_RESPONSE));
		...
		case AT_ID.ID_SET_RADIO_FREQ: {
			int freq = (((int)(caninfo[5]&0xFF))<<16) 
					+ (((int)(caninfo[6]&0xFF))<<8) 
					+ ((int)(caninfo[7]&0xFF));
			ATEvent event = new ATEvent();
			event.type = ATConst.Radio.TYPE_RADIO;
			event.cmd = ATConst.Radio.CMD_RADIO_SET_FREQ;
			event.info = getFreq(freq);
			return event;
		}
...
//			mService.sendData(genCANData(data, data.length, TYPE_POSITIVE_RESPONSE));
	 ...
	 * @return
	 */
	private byte[] onProcessRevert(final int type, final int cmd, final String info) {
		Log.i(TAG, "Revert type=" + type + ", cmd = " + cmd + "info = " + info);
		switch (type) {
		case ATConst.Logic.TYPE_LOGIC: {
			switch (cmd) {
			case ATConst.Logic.CMD_LOGIC_SET_SOURCE: {
				byte[] data = { 0x18, 0x01, 0x00 };
				if ("ok".equals(info)) {
					data[2] = 0x01;
				}
				return genCANData(data, data.length);
			}
			case ATConst.Logic.CMD_LOGIC_SET_VOL: {
				byte[] data = {0x19, 0x01, 0x00};
				if ("ok".equals(info)) {
					data[2] = 0x01;
				}
				return genCANData(data, data.length);
		...
		case ATConst.System.TYPE_SYSTEM: {
			...
			case ATConst.System.CMD_SYSTEM_GET_SN: {
				byte[] data = { 0x21, 0x09, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
		...
		case ATConst.Settings.TYPE_SETTINGS: {
			...
			case ATConst.Settings.CMD_SETTINGS_GET_WIFI_ADDR: {
				byte[] data = { 0x24, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
				if (!TextUtils.isEmpty(info)){
					byte[] bytes = info.getBytes();
		...
		case ATConst.BT.TYPE_BT: {
			...
			case ATConst.BT.CMD_BT_GET_BT_ADDR: {
				byte[] data = { 0x1F, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
				if (!TextUtils.isEmpty(info)){
					String[] split = info.split(":");
					if (split != null){
						for (int i = 0; i < split.length; i++){
							data[2+i] = (byte) Integer.parseInt(split[i], 16);
						}
					}
					/*byte[] bytes = info.getBytes();
					for (int i = 0; i < bytes.length && i < 12; i++){
		...
		case ATConst.GPS.TYPE_GPS: {
			...
			case ATConst.GPS.CMD_GPS_GET_GPS_TIME: {
				byte[] data = { 0x14, 0x06, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
				if (!TextUtils.isEmpty(info) && info.contains("-")){
					String[] split = info.split("-");
					try {
	...
	private String getFreq(int freq){
		if (freq <= 1620){
			return "" + freq;
		}
		else{
