...
public class SuperImageView extends ImageView{
    ...
    private static final String TAG = "SuperImageView";
    int mode = NONE;
...
//        Log.i(TAG, "viewH="+viewH);
//        Log.i(TAG, "viewW="+viewW);
//        Log.i(TAG, "imageH="+imageH);
//        Log.i(TAG, "imageW="+imageW);
...
//        Log.i(TAG, "deltaY="+deltaY);
    ...
    private void fixTranslationAndShowImg() {
        RectF rect = new RectF(0, 0, imageW, imageH);
        matrix.mapRect(rect);
    ...
    public boolean onTouchEvent(MotionEvent event) {
    	//Log.d(TAG,"onTouchEvent event.getAction() "+ (event.getAction()& MotionEvent.ACTION_MASK));
...
//                        doubleClick(pA.x, pA.y);
                    ...
                    lastClickTime = now;
	               	if(this.showListener != null){
	                     this.showListener.singleClickEvent();
	                     changeImage = true;
	                 }
                }
    ...
    public boolean ZoomOut(){
        ...
        _toScale = curScale - 0.2f;
        if(_toScale < minScale){
        	_toScale = minScale;
        }else if(_toScale == minScale){
        	Log.d(TAG,"ZoomOut miniscale.");
        	return false;
        }
        float toScale = _toScale / curScale;
        matrix.postScale(toScale, toScale, viewW/2, viewH/2);
        fixScale();
        fixTranslation();
        setImageMatrix(matrix);
    	return true;
    ...
    public boolean ZoomIn(){
        ...
        _toScale = curScale + 0.2f;
        if(_toScale > MAX_SCALE){
        	_toScale = MAX_SCALE;
        }else if(_toScale == MAX_SCALE){
        	Log.d(TAG,"ZoomIn maxscale.");
        	return false;
        }
        float toScale = Math.min(_toScale, MAX_SCALE)/ curScale ;
        matrix.postScale(toScale, toScale, viewW/2, viewH/2);
        fixScale();
        fixTranslation();
        setImageMatrix(matrix);
