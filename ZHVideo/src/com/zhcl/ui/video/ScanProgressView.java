...
public class ScanProgressView extends RelativeLayout implements ThreadForever{
	...
	private void uIShow(String tip){
		int scale = FileScanner.getInstance(null).getScanedDirPercent(null);
		if(scale < 100){
			type.setText("[" + tip + "] 扫描文件夹；下一步：视频信息处理");
			...
			Log.e(tag, "分类处理中····");
			type.setText("[" + tip + "] 视频信息处理中···");  
			int scale2 = VideoManager.getIntance(null).getCurrentPercent();
			info.setText("进度：" + scale2 + "%" + " 处理文件：" + getSimpleName(VideoManager.getIntance(null).getCurrentSongPath()));
		}
	}
